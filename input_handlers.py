...
class MainGameEventHandler(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if (key == tcod.event.K_PERIOD) and modifier & (
            ...
            return actions.TakeStairsAction(player, False)
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[MainGameEventHandler]:
        ...
        if event.sym in CURSOR_Y_KEYS:
            ...
            if adjust < 0 and self.cursor == 0:
                ...
                self.cursor = max(0, min(self.cursor + adjust, self.log_length - 1)) # TODO: adjust to clamp command
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        the letter to select them. Will move to a different position based on where
        the player is located, so the player can always see where they are.
        """
        super().on_render(console)
        ...
        if number_of_items_in_inventory > 0:
            for i, item in enumerate(self.engine.player.inventory.items):
                ...
                console.print(x + 1, y + i + 1, f"({item_key}){equipped_string} {item.name}")
        else:
